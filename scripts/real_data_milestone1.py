...
class RealDataMilestone1:
    ...
    def generate_student_batch(self, batch_num, university_profiles, total_batches):
        ...
        for uni_idx, uni_name in enumerate(self.ipeds_universities):
            ...
            for student_local_id in range(uni_students):
                ...
                # Generate 8-12 subject records per student across time period
                num_subjects = np.random.randint(8, 13)
                selected_subjects = np.random.choice(self.subjects, num_subjects, replace=False)
                
                for subject in selected_subjects:
                    # Random year in our range
                    year = np.random.randint(self.start_year, self.end_year + 1)
                    ...
                    records.append({
                        ...
                        'name': name,
                        'university': uni_name,
                        'university_state': uni_profile['state'],
                        'university_type': uni_profile['type'],
                        'subject': subject,
                        'score': score,
                        'grade': grade,
    ...
    def process_batch_data(self, batch_records, batch_num):
        ...
        # Save batch file
        batch_file = os.path.join(self.data_dir, f"students_batch_{batch_num:02d}_100K.csv")
        df_batch.to_csv(batch_file, index=False)
        
        logger.info(f"Batch {batch_num} saved: {batch_file} ({len(df_batch):,} records)")
        return df_batch
