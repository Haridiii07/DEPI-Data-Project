...
if conn:
    ...
    with tab4:
        ...
        with col_search:
            student_number = st.number_input("Enter Student Number (1-1,000,000)", 
                                            min_value=1, 
                                            max_value=1000000, 
                                            step=1,
                                            value=None)
        ...
        if student_number:
            student_id_formatted = f"UNI00_STU{student_number:08d}"
            student_query = "SELECT * FROM dim_student WHERE student_id = ?"
            student_info = conn.execute(student_query, [student_id_formatted]).fetchdf()
            
            if not student_info.empty:
                with col_info:
                    st.success(f"**Found:** {student_info['student_name'].iloc[0]}")
                ...
                """
                history_df = conn.execute(history_query, [student_id_formatted]).fetchdf()
            ...
            st.info("Enter a student number to view their academic profile.")
