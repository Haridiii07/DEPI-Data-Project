...
If you already have the files, ensure you are at the project root that contains `README.md`, `scripts/`, and `data/`.
...
What it does:
- Scans `data/milestone1_real/` for `students_batch_01_100K_cleaned.parquet` … `students_batch_10_100K_cleaned.parquet`
- Concatenates batches in numeric order
- Saves `data/milestone1_real/cleaned_students.parquet`
...
- Creates `scripts/student_performance.duckdb`
...
- Prefer forward slashes (`scripts/assemble_dataset.py`) in documentation; PowerShell accepts them.
