...
df = pd.read_csv('data/milestone1_real/cleaned_students.csv')  # ~30 seconds
...
3. Script behavior:
   - Scans `data/milestone1_real/students_batch_XX_100K_cleaned.csv`
...
`scripts/assemble_dataset.py` concatenates the Parquet batches into a single file (`cleaned_students.parquet`). Highlights:
...
```sql
SELECT *
FROM 'data/milestone1_real/cleaned_students.parquet'
LIMIT 10;
```
