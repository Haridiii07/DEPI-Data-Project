...
- **Fix:** Run commands from the project root. Verify the data folder contains Parquet batches. Re-run `scripts/convert_to_parquet.py` and `scripts/assemble_dataset.py` if necessary.
...
- Ensure DuckDB file exists (`scripts/student_performance.duckdb`). If not, run `python scripts/build_database.py`.
- Use `python -m streamlit run dashboard/app.py` from repo root.
...
- Windows PowerShell accepts forward slashes (recommended): `python scripts/assemble_dataset.py`
- Some commands require quoting: `jupyter notebook "scripts\Milestone2_3_SQL_and_Visualizations.ipynb"`
...
```powershell
Compress-Archive -Path data/milestone1_real/sample_100K_students.csv `
  -DestinationPath data/milestone1_real/sample_100K_students.zip -Force
```
